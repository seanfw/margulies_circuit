...
from numba_code import dot_np, matmul
import brian2
import numpy as np
...
for i_t in range(1,num_iterations):
    ...
    if c.get('long_range') == 'True':
        conn_delay = np.zeros(shape=(np.shape(tract_connectivity)))
        conn_delay = matmul(tract_connectivity,S_tract_delay,conn_delay)
        pm['I_longRange_NMDA'][i_t-1,:,0] = pl['g_E_longRange'] * np.sum(conn_delay,axis=1)

    # mid range NMDA (along surface), with a 7 timestep delay
    if c.get('mid_range') == 'True':
        pm['I_midRange_NMDA'][i_t-1,:,:] = pl['g_E_midRange'] * dot_np(surface_connectivity,pm['S_NMDA'][i_t-pl['midrange_delay'],:,:])
